...
function FeedbackModal({ onClose, listing, onUpload }: { onClose: () => void; listing?: Listing; onUpload?: () => void }) {
  ...
  async function submit(event:FormEvent){event.preventDefault();if(!rating){setError("Please select a rating.");return}setBusy(true);setError("");const storedMessage=isTestReel?`[Test reel ${listing?.id}: ${listing?.title}] ${message}`:message;const {error}=await supabase.functions.invoke("submit-feedback",{body:{rating,category:isTestReel?"property_search":category,message:storedMessage,email,mobile,website:""}});setBusy(false);if(error)setError("Your feedback could not be sent. Please check the details and try again.");else setSent(true)}
