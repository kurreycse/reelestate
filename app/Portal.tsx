  ...
  async function requestOtp(purpose = otpPurpose) {
    ...
    if (error) {
      ...
      if (responseError === "account_exists" && purpose === "register") {
        ...
        responseError === "account_not_found" &&
        (purpose === "login" || purpose === "reset")
      ) {
...
function FeedbackModal({ onClose }: { onClose: () => void }) {
  const [rating,setRating]=useState(0);const [category,setCategory]=useState("general");const [message,setMessage]=useState("");const [email,setEmail]=useState("");const [mobile,setMobile]=useState("");const [busy,setBusy]=useState(false);const [error,setError]=useState("");const [sent,setSent]=useState(false);
  async function submit(event:FormEvent){event.preventDefault();if(!rating){setError("Please select a rating.");return}setBusy(true);setError("");const {error}=await supabase.functions.invoke("submit-feedback",{body:{rating,category,message,email,mobile,website:""}});setBusy(false);if(error)setError("Your feedback could not be sent. Please check the details and try again.");else setSent(true)}
