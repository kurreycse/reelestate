...
Deno.serve(async request=>{
  ...
  try{
    ...
    const id=String(data.id||"");const videoPath=String(data.video_path||"");const posterPath=String(data.poster_path||"");if(!uuid.test(id)||!new RegExp(`^${user.id}/${id}\\.(mp4|mov)$`,"i").test(videoPath)||posterPath!==`${user.id}/${id}.jpg`)return json({error:"invalid_media_path"},400,origin);const {data:existing}=await admin.from("listings").select("owner_id,status").eq("id",id).maybeSingle();if(existing&&(existing.owner_id!==user.id||existing.status!=="rejected"))return json({error:"resubmission_not_allowed"},409,origin);
    ...
    const {data:listingId,error}=await admin.rpc(existing?"resubmit_validated_listing":"create_validated_listing",{p_owner_id:user.id,p_data:data});if(error)throw error;await notifyListing(admin,"listing_submitted",{id:String(listingId),owner_id:user.id,title,city,locality});return json({accepted:true,listing_id:listingId,resubmitted:Boolean(existing)},201,origin);
